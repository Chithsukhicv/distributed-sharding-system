...
DB_NAME = os.environ.get('DB_NAME', 'shard_1_db')
...
def health():
    return jsonify({"status": "running", "port": PORT, "database": DB_NAME})
...
def put_student():
    ...
    try:
        ...
        return jsonify({"status": "success", "message": f"student {data['student_id']} stored", "shard": DB_NAME}), 201
...
def count_students():
    ...
    return jsonify({"status": "success", "count": count, "shard": DB_NAME})
