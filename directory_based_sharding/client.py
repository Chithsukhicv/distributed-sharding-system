...
def performance_test(num_queries=500, max_workers=10):
    ...
    print(f"\n{'='*40}")
    print(f"PERFORMANCE RESULTS - DIRECTORY BASED SHARDING")
    print(f"{'='*40}")
    print(f"Total queries    : {num_queries}")
...
def compare_results(single_db_avg, sharded_avg):
    ...
    print(f"Directory avg      : {sharded_avg:.4f} ms")
...
if __name__ == '__main__':
    ...
    # Read the dynamic single DB average
    try:
        ...
        print("\nNote: single_db_avg.txt not found. Run single_db_instance/client.py first.")
