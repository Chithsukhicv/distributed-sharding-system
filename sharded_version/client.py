...
def performance_test(num_queries=500, max_workers=50):
    ...
    print(f"Sending requests through central server → sharded backends")

    query_times = []
    random_ids = [random.randint(1, 10_000_000) for _ in range(num_queries)]
    ...
    print(f"\n{'='*40}")
    print(f"PERFORMANCE RESULTS - SHARDED DB")
    print(f"{'='*40}")
...
def compare_results(single_db_avg, sharded_avg):
    ...
    print(f"Sharded average    : {sharded_avg:.4f} ms")
...
if __name__ == '__main__':
    ...
    single_db_avg = 0
    try:
        metrics_path = os.path.join(os.path.dirname(__file__), "..", "single_db_avg.txt")
