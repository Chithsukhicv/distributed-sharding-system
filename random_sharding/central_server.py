...
def health():
    return jsonify({"status": "running", "role": "central_server_random", "port": 5000})
...
async def put_student():
    ...
    # RANDOM ROUTING: Pick a random shard!
    shard = random.choice([0, 1, 2])
...
async def get_student(student_id):
    ...
    # Since placement was random, we don't know which shard holds the data.
